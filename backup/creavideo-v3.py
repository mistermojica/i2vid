...
def create_video(num_elements, output_filename, download_path, property_name, thumb_filename):
    ...
    for i in range(0, num_elements, 2):
        ...
        # Leer las imágenes
        img_clip1 = ImageClip(f"{download_path}/images/image_{i}.jpg").set_duration(combined_duration)
        # img_clip1 = img_clip1.resize((1080, 1920), Image.Resampling.LANCZOS)
        img_clip1 = img_clip1.resize((1080, int(1080 * img_clip1.height / img_clip1.width)), Image.Resampling.LANCZOS)

        # original = ImageClip('img.png')
        # resized = original.resize(0.5)xc
        ...
        if img_clip2:
            ...
            # img_clip2 = img_clip2.resize((1080, 1920), Image.Resampling.LANCZOS)
            img_clip2 = img_clip2.resize((1080, int(1080 * img_clip2.height / img_clip2.width)), Image.Resampling.LANCZOS)
    ...
    centered_img_clip = ImageClip("./images/LuxuryRoamersHD.png").set_duration(final_clip.duration)
    ...
    # Ajusta tu línea de código usando el filtro adecuado
    centered_img_clip = centered_img_clip.resize((int(final_clip.w * 0.85), int(final_clip.h * 0.85)), resampling_filter).set_position("center")

    # centered_img_clip = centered_img_clip.resize((final_clip.w * 0.85, final_clip.h * 0.85), Image.Resampling.LANCZOS).set_position("center")
