...
# Añadir el logo al video
def add_logo(logo_path, video_clip):
    logo = (ImageClip(logo_path)
            .set_duration(video_clip.duration)
            .resize(width=video_clip.w * 0.8)  # Ajustar el tamaño del logo al 30% del ancho del video
            .set_pos(("center", 100)))  # Agregar un margen de 50 píxeles desde la parte superior
    return logo
...
def add_options(options, video_clip, options_font_path, margin=170, top_margin=1000):
    ...
    for i, option in enumerate(options):
        ...
        option_text_clip = (TextClip(option, fontsize=70, color='black', font=options_font_path)
                            .set_duration(video_clip.duration)
                            .set_position((margin + circle_radius * 2 + 20, y_positions[i] + 10)))
...
def add_account_text(account_text, video_clip):
    account_clip = (TextClip(account_text, fontsize=30, color='white')
                    .set_duration(video_clip.duration)
...
def compose_video(background_clip, logo_clip, question_clip, options_clips, account_clip, narration_audio, sound_effects, output_file):
    ...
    final_clip.write_videofile("./trivia/videos/output.mp4", codec='libx264', fps=24, preset='ultrafast')
...
def generate_trivia_video(background_video_path, logo_path, question_text, options, correct_option_index, account_text, narration_text, tictac_sound_path, output_file, question_font_path, options_font_path):
    narration_audio_file = f"./trivia/audios/{uuidcode}.mp3"
    ...
    account_clip = add_account_text(account_text, background_clip)
...
generate_trivia_video(
    background_video_path="./trivia/assets/videos/background1.mp4",
    logo_path="./trivia/assets/images/logo.png",
    ...
    tictac_sound_path="./trivia/assets/audios/clock.mp3",
    output_file=f"./trivia/videos/{uuidcode}.mp4",
    question_font_path="./trivia/assets/fonts/TT-Milks-Casual-Pie-Trial-Base.otf",  # Ruta a la fuente para la pregunta
    options_font_path="./trivia/assets/fonts/Sniglet-Regular.ttf"  # Ruta a la fuente para las opciones
)
